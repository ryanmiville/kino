...
import kino/stage.{
  type BufferStrategy, type Produce, Done, KeepFirst, KeepLast, Next,
}
...
pub fn start(
  ...
  actor.start_spec(actor.Spec(
    init: fn() {
      ...
      let state =
        internal.ProducerConsumerState(
          self:,
          consumer_self:,
          producer_self:,
          selector:,
          ...
          events: internal.Events(queue: deque.new(), demand: 0),
          handle_events: builder.handle_events,
        )
      actor.Ready(state, selector)
    },
    loop: internal.pc_on_message,
